...
// through OpenAI-compatible upstreams that only implement /v1/chat/completions.
func (s *OpenAIGatewayService) forwardAnthropicViaRawChatCompletions(
	...
	if responsesReq.Reasoning != nil && strings.TrimSpace(responsesReq.Reasoning.Effort) != "" {
		v := strings.TrimSpace(responsesReq.Reasoning.Effort)
		reasoningEffort = &v
	}
	var serviceTier *string
	if strings.TrimSpace(responsesReq.ServiceTier) != "" {
		v := strings.TrimSpace(responsesReq.ServiceTier)
		serviceTier = &v
	}

	logger.L().Debug("openai messages chat fallback: forwarding without responses endpoint",
...
func (s *OpenAIGatewayService) bufferChatCompletionsAsAnthropic(
	...
	if value := openAIResponsesAuditText(responsesResp.Output); strings.TrimSpace(value) != "" {
...
func (s *OpenAIGatewayService) streamChatCompletionsAsAnthropic(
	...
	processResponsesEvents := func(events []apicompat.ResponsesStreamEvent) {
		for _, event := range events {
			if data, err := json.Marshal(event); err == nil {
				appendOpenAIAuditText(&auditText, data)
			}
	...
	for scanner.Scan() {
		...
		processResponsesEvents(apicompat.ChatCompletionsChunkToResponsesEvents(&chunk, chatState))
	}
	...
	if err := scanner.Err(); err != nil {
		...
		return &OpenAIForwardResult{
			RequestID:       requestID,
			ResponseID:      chatState.ResponseID,
			Usage:           usage,
			Model:           originalModel,
			BillingModel:    billingModel,
			UpstreamModel:   upstreamModel,
	...
	processResponsesEvents(apicompat.FinalizeChatCompletionsResponsesStream(chatState))
